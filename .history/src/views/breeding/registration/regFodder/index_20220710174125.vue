  ...
  setup() {
    ...
    const numChange = (index) => {
      if (addInfo.number[index]) {
        addInfo.weight[index] =
          parseInt(addInfo.number[index]) * parseInt(addInfo.size[index]);
      }
    };
    ...
    const editNumChange = () => {
      if (editInfo.value.number) {
        editInfo.value.weight =
          parseInt(editInfo.value.number) * parseInt(editInfo.value.size);
      }
    };
    const formRules = ref({
    ...
    const addFodder = () => {
      proxy.$refs.addRef.validate(async (valid) => {
        ...
        await proxy.$API.regOutCage.addfeed.post(addInfo.value).then((res) => {
          if (res.success) {
            proxy.$message({
              message: "添加成功",
              type: "success",
    ...
    const updateFodder = () => {
      proxy.$refs.editRef.validate(async (valid) => {
        ...
        await proxy.$API.regOutCage.modifyfeed
          .post(editInfo.value)
          .then((res) => {
            if (res.success) {
              proxy.$message({
    ...
    const removeFodder = async (id) => {
      ...
      await proxy.$API.regOutCage.deletefeed.post(id).then((res) => {
        if (res.success) {
          proxy.$message({
            message: "删除成功",
            type: "success",
