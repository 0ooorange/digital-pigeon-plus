...
export default defineComponent({
  ...
  setup() {
    ...
    const api = proxy.$API.regFodder.getallocatefeed;
    const params = {
    ...
    const addFodder = () => {
      proxy.$refs.addRef.validate(async (valid) => {
        ...
        await proxy.$API.regFodder.addallocatefeed.post(addInfo.value).then((res) => {
          if (res.success) {
            proxy.$message({
              message: "添加成功",
              type: "success",
    ...
    const updateFodder = () => {
      proxy.$refs.editRef.validate(async (valid) => {
        ...
        await proxy.$API.regFodder.modifyallocatefeed
          .post(editInfo.value)
          .then((res) => {
            if (res.success) {
              proxy.$message({
    ...
    const removeFodder = async (id) => {
      ...
      await proxy.$API.regFodder.deleteallocatefeed.post(id).then((res) => {
        if (res.success) {
          proxy.$message({
            message: "删除成功",
            type: "success",
